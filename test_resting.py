...
for j in range(rkf.get_n_splits()):
    ...
    for k in to_do:
        ...
        while test_nan:
            ...
            history = model.fit(X_train, Y_train, nb_epoch=epochs, batch_size=batch_size, callbacks=[early_stopping],
            ...
            with open(fold_model + outfile + ".json", "w") as json_file:
                json_file.write(model_json)
            # serialize weights to HDF5
            model.save_weights(fold_model + outfile + ".h5")
            # save dataset in npz format
            print(X_test.shape)
            np.savez(fold_model + outfile + ".npz", nepoch=nepoch, batch_size=batch_size, cputime=cputime, drop=drop,
                     rois_ids=rois_ids)
