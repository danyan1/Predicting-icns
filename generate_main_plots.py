...
fpr_model = np.array(
    [[sum(list_res[i][:, 0][list_res[i][:, 1] != clas_id] == clas_id) / float(sum(list_res[i][:, 1] != clas_id)) \
...
rois_ind_all = [np.load('./results/task_tr_rest_tr/task_tr_resting_ts_' + str(fold) + '.npz')['rois_ids'] for fold in
                range(50)]
